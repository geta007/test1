...
resource "azurerm_public_ip" "publicdomainip" {
 name                         = "domainipuser14final"
 location                     = "koreasouth"
 resource_group_name          = "${azurerm_resource_group.rg.name}"
 allocation_method            = "Static"
 domain_name_label            = "user14finalskcncazure4"
}

resource "azurerm_public_ip" "rdpip" {
    name = "rdpipuser14final${count.index}"
    location = "koreasouth"
    resource_group_name = "${azurerm_resource_group.rg.name}"
...
resource "azurerm_network_interface" "nic" {
  name                = "user14finalnic${count.index}"
  location            = "koreasouth"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  network_security_group_id = "${azurerm_network_security_group.secGroup.id}"  
  count               = 2
...
resource "azurerm_virtual_machine" "vm" {
  name                  = "user14vmfinal${count.index}"
  location              = "koreasouth"
  resource_group_name   = "${azurerm_resource_group.rg.name}"
 availability_set_id   = "${azurerm_availability_set.avset.id}"
  ...
  storage_os_disk {
        name = "user14finalosdisk${count.index}"
        caching = "ReadWrite"
  ...
  os_profile {
        computer_name = "user14vmfinal${count.index}"
        admin_username = "azureuser14"
